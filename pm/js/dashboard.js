...
(() => {
  ...
  async function load() {
    try {
      [projects, taskSummaries] = await Promise.all([API.getProjects(), API.getTaskSummaries()]);
      render();
    } catch (e) {
      grid.innerHTML = "";
      UI.toast(e.message);
    ...
    if (visible.length === 0) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column: 1 / -1;">
          ...
          <p>Create your first project — one per client or internal workstream.</p>
          <button class="btn btn-primary" id="empty-new-btn">+ New Project</button>
        </div>`;
