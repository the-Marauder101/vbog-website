-- Migration 28: V10c — Fix pravah_kpi_dashboard (second wrong column)
--
-- Migration 27 corrected pravah_training.trainer_id -> trainer_uid. Applying
-- it revealed a second reference of exactly the same kind in the same
-- function:
--
--   ERROR 42703: column t.started_at does not exist
--   HINT: Perhaps you meant to reference the column "t.started_on".
--
-- Rather than fix these one at a time, every aliased column reference in the
-- function was then checked against information_schema. Twenty-six
-- references across eleven tables were verified; t.started_at was the only
-- remaining one that does not resolve. This migration should therefore be
-- the last of its kind for this function.
--
-- Verified as resolving: pravah_training (trainer_uid, status, started_on),
-- pravah_targets (period_start, target_unit, target_value), pravah_insights
-- (author_uid, validation_status), pravah_interventions (owner_uid,
-- effectiveness), pravah_actions (owner_uid, completed_at, due_on),
-- pravah_client_checkins (satisfaction, created_by, occurred_at),
-- pravah_selection_reviews (reviewer_uid, review_date, technical_decision,
-- client_final_decision), placement_outcomes (checkpoint, retained),
-- pravah_performance_reports (verified_cash_collected, created_by),
-- pravah_company_targets (target_value), pravah_kpi_definitions (sort_order).
