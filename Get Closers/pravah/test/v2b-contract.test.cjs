...
const hotfixSql = fs.readFileSync(path.join(root, "supabase/07_v2b_candidate_status_time_fix.sql"), "utf8");
...
assert.match(sql, /status_changed_at\s+timestamptz/);
assert.match(hotfixSql, /add column if not exists status_changed_at timestamptz/);
assert.match(hotfixSql, /drop view if exists pravah_v_candidate_sync_inbox/);
assert.match(hotfixSql, /create view pravah_v_candidate_sync_inbox/);
