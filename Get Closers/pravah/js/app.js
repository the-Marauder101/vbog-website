(function () {
  ...
  async function submitDynamicForm(event) {
    ...
    try {
      ...
      if (type === "invite-client") await api.rpc("pravah_create_portal_user", { p_email: values.email.trim(), p_password: values.password, p_role: values.role, p_client_id: values.client_id, p_display_name: valueOrNull(values.display_name) });
      if (type === "invite-closer") { const placement = state.placements.find((p) => p.placement_id === values.placement_id); await api.rpc("pravah_create_portal_user", { p_email: values.email.trim(), p_password: values.password, p_role: "closer", p_client_id: placement?.client_id, p_display_name: valueOrNull(values.display_name), p_placement_id: values.placement_id }); }
      if (type === "add-staff") await api.rpc("pravah_create_staff_user", { p_email: values.email.trim(), p_role: values.role, p_display_name: valueOrNull(values.display_name), p_password: values.password });
  ...
  // Session-based password change: no SMTP, so an emailed reset can never arrive.
  document.querySelectorAll("[data-change-password]").forEach((b) => b.addEventListener("click", () => {
    byId("pw-error").textContent = ""; byId("pw-new").value = ""; byId("pw-confirm").value = "";
    byId("pw-modal").hidden = false;
  }));
  document.querySelectorAll("[data-close-pw]").forEach((b) => b.addEventListener("click", () => { byId("pw-modal").hidden = true; }));
  byId("pw-modal").addEventListener("click", (e) => { if (e.target === byId("pw-modal")) byId("pw-modal").hidden = true; });
  byId("pw-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    const a = byId("pw-new").value, b = byId("pw-confirm").value, err = byId("pw-error");
    err.textContent = "";
    if (a !== b) { err.textContent = "Passwords do not match."; return; }
    try { await api.changePassword(a); byId("pw-modal").hidden = true; showToast("Password updated"); }
