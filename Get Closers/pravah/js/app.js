(function () {
  ...
  async function submitDynamicForm(event) {
    ...
    try {
      ...
      if (type === "add-staff") await api.rpc("pravah_invite_staff", { p_email: values.email.trim(), p_role: values.role, p_display_name: valueOrNull(values.display_name) });
