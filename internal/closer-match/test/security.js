...
suite("SECURITY SUITE", 8104, async ({ p, base, E, P, check, errs }) => {
  ...
  const INTERNAL_ONLY = {
    ...
    fit_from_levels: "PRD 9.3 itself — called by compute_matches and one_fit",
    ask_levels_merged: "every submitted round as one answer set, for one_fit",
    candidate_levels: "merges the interview and the questionnaire per dimension",
    levels_flat: "drops the provenance so the two scoring functions see plain levels",
    one_fit: "the single point — reached through get_candidate_detail's payload",
    refresh_stale_matches: "called by loadQueue before the queue is read",
