...
| File | What it holds |
|---|---|
| `styles.css` | Entry point. `@import`s everything below — link this one file. |
| `tokens/colors.css` | The 8 brand colours plus derived rules, with usage meanings. |
| `tokens/typography.css` | The 3 type roles with their tracking and leading ranges. |
| `tokens/layout.css` | Shell widths, gutters, section rhythm, hard-shadow and grid tokens. |
| `vbog-base.css` | The component recipes and signature devices as real classes. |
| `guidelines/vbog-brand-kit.md` | Full brand specification — voice, positioning, imagery, preflight. |
| `guidelines/vbog-logo.svg` | The only permitted logo. |
| `_ds_bundle.js` | Intentionally empty — this kit ships no React components. |

Every value traces to `VBOG_BRAND_KIT.md`; the class implementations are extracted
from the shipped `revenue-refusal.css`, so they are built VBOG patterns, not proposals.
